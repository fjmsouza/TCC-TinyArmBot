...
class Geometric:
    ...
    def translation(self, a=1, b=0, c=400, d=0, e=1, f=250):
        rows, cols, _ = self.frame.shape
        M = np.float32([[a, b, c], [d, e, f]])
        translate = cv2.warpAffine(self.frame, M, (cols, rows))

        return translate

    def rotation(self, a=1, b=2.0, c=1, d=2.0, e=250, f=2):
        rows, cols, _ = self.frame.shape
        M = cv2.getRotationMatrix2D(((cols - a) / b, (rows - c) / d), e, f)
        rotation = cv2.warpAffine(self.frame, M, (cols, rows))
    ...
    def rectify(self):
        try:
            ...
            #borramento
            image_blur = cv2.GaussianBlur(self.frame, (3, 3), -1)
            ...
            cv2.imwrite('Picture1blur.png', image_blur)
            cv2.imwrite('Picture2gray.png', image_gray)
            # cv2.imwrite('Picture3laplace.png', image_laplace)
            cv2.imwrite('Picture4canny.png', image_canny)
            # cv2.imwrite('Picture4sobel.png', image_sobel)
            ...
            for c in contours:
                ...
                rect = cv2.rectangle(self.frame, (x, y), (x + w, y + h), color=(0, 255, 0), thickness=2)
